...
class Athena(arachne.Arachne):
    ...
    def localize(self, model, input_neg, target_data_dir: Path, verbose=1):
        ...
        for label in tqdm(req, desc="Localizing for repair data"):
            ...
            pool = self._compute_forward_impact(reshaped_model, input_neg, candidates, num_grad)
    ...
    def load_input_neg(self, input_dir):
        ...
        if len(repair_images) == 0:
            raise ValueError(
                "No negative dataset is loaded. " "Be sure to set appropreate `repair_priority`."
            )
    ...
    def _sample_positive_inputs(self, input_pos):
        ...
        if self.num_input_pos_sampled is None:
            return input_pos
        sample = np.random.default_rng().choice(len(input_pos[0]), self.num_input_pos_sampled)
    ...
    def _criterion(self, model, location, input_pos, input_neg):
        ...
        model
            Subject DNN model
        location
            Consists of a neural weight value to mutate, an index of a layer of the model,
            and a neural weight position (i, j) on the layer
        input_pos
            Positive inputs sampled
        input_neg
            Negative inputs targeted

        Returns
        -------
        fitness, n_patched and n_intact
        ...
        if 0 < len(self.input_protected[0]):
            results = model.predict(self.input_protected[0], verbose=0, batch_size=self.batch_size)
            n_protected = 0
    ...
    def optimize(
        ...
        for label in weights_repaired:
            final_model = self._copy_location_to_weights(weights_repaired[label], final_model)
